...
_zsh_autosuggest_modify() {
        ...
        # Get a new suggestion if the buffer is not empty after modification        if (( $#BUFFER > 0 )); then
...
_zsh_autosuggest_accept() {
        ...
        # If we're not in a valid state to accept a suggestion, just run the        # original widget and bail out
...
_zsh_autosuggest_strategy_history() {
        ...
        # this string as a pattern to search the $history associative array.        # - (#m) globbing flag enables setting references for match data
        ...
        # Give the first history item matching the pattern as the suggestion        # - (r) subscript flag makes the pattern match on values
...
#
# Note that this strategy won't work as expected with ZSH options that don't# preserve the history order such as `HIST_IGNORE_ALL_DUPS` or
...
add-zsh-hook precmd _zsh_autosuggest_star
